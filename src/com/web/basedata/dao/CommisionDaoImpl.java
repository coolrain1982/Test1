...
public class CommisionDaoImpl implements CommisionDao {
	...
	public List<Commision> getCommision(int type) {
		String hql = "select e from Commision "
				+ "e where e.type = :type and e.date = (select max(date) from Commision f where e.type = f.type"
				+ " and e.srv_mode=f.srv_mode group by e.srv_mode,e.type)";
	...
	public List<Commision> getCommision(int startIdx, int size) {
		String hql = String.format("select new com.web.entity.Commision(c.type, c.srv_type, c.srv_mode, c.fee,c.fee1,c.fee2, c.date) from Commision c order by c.date desc");
